let chalk,version;_b94‍.x([["showOptions",()=>showOptions],["showHelp",()=>showHelp],["showVersion",()=>showVersion]]);_b94‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);_b94‍.w("../package.json",[["default",["version"],function(v){version=v}]]);



       function showOptions(options) {
    process.stdout.write("Your ");
    process.stdout.write(chalk.yellow("options"));
    process.stdout.write(" are:\n");
    process.stdout.write("Root folder to search CSS's, HTML's and PHP's: ");
    _b94‍.g.console.log(chalk.bold.green(options.folderToImplode));
    process.stdout.write("Audit (not performing any action, just showing the unused classes): ");
    _b94‍.g.console.log(chalk.bold.green(options.audit));
    process.stdout.write("Fix (removed unused CSS ids and classes): ");
    _b94‍.g.console.log(chalk.bold.green(options.fix) + chalk.white('\n'));
}

       function showHelp() {
    _b94‍.g.console.log(chalk.green('\nImplodeCss') + chalk.gray('  v' + version.version));

    _b94‍.g.console.log('\n' + chalk.bold('Usage') + ': implodeCss ' + chalk.magenta('[folder] [options]\n'));

    _b94‍.g.console.log(chalk.bold('Global options:\n'));
    console.log('  -y, --yes\t\tIgnore all options and use the default ones.');
    console.log('  -a, --audit\t\tOutput unused CSS selectors.');
    console.log('  -f, --fix\t\tFix unused CSS selectors and output the result.');
    console.log('  -h, --help\t\tOutput usage information.');
    console.log('  -v, --version\t\tOutput package version.');

    _b94‍.g.console.log(chalk.bold('\nExample of use:'));
    console.log('  implodeCss /var/www/html/myHost --fix');

    _b94‍.g.console.log(chalk.bold('\nHint:'));
    _b94‍.g.console.log('  If you do not know how to use the package, simply do: \'' +
        chalk.bold('ImplodeCss') + '\' (without arguments) and the package will prompt you the needed options');
}

       function showVersion() {
    _b94‍.g.console.log(chalk.bold.gray(version.version));
}

exports.showHelp = showHelp;
exports.showOptions = showOptions;
exports.showVersion = showVersion;
