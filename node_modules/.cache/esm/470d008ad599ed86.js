let chalk,exitCodes,getArrayHtmlPhpPaths,findFilesInDir,getIdsReferencedInHtml,getClassesReferencedInHtml,getUnusedCss,testPathFile,runHttpServer,writeDataFile,OptionsClass;_d59‍.x([["auditCode",()=>auditCode],["fixCode",()=>fixCode]]);_d59‍.w("chalk",[["default",["chalk"],function(v){chalk=v}]]);_d59‍.w("./Exceptions/exitCodes",[["default",["exitCodes"],function(v){exitCodes=v}]]);_d59‍.w("./controller/getFiles",[["getArrayHtmlPhpPaths",["getArrayHtmlPhpPaths"],function(v){getArrayHtmlPhpPaths=v}],["findFilesInDir",["findFilesInDir"],function(v){findFilesInDir=v}]]);_d59‍.w("./controller/getCssReferenced",[["getIdsReferencedInHtml",["getIdsReferencedInHtml"],function(v){getIdsReferencedInHtml=v}],["getClassesReferencedInHtml",["getClassesReferencedInHtml"],function(v){getClassesReferencedInHtml=v}]]);_d59‍.w("./controller/geCssUnused",[["default",["getUnusedCss"],function(v){getUnusedCss=v}]]);_d59‍.w("./testPath",[["default",["testPathFile"],function(v){testPathFile=v}]]);_d59‍.w("./server/createServer",[["runHttpServer",["runHttpServer"],function(v){runHttpServer=v}],["writeDataFile",["writeDataFile"],function(v){writeDataFile=v}]]);_d59‍.w("./cli/options.class",[["default",["OptionsClass"],function(v){OptionsClass=v}]]);/**
 ...
 * @return {void}
 */
       async function auditCode(folderToImplode, port) {
    let aSingleton = (new OptionsClass()).singletonFunction;
    _d59‍.g.console.log(aSingleton.getDebug());
 ...
 * @return {void}
 */
       async function fixCode(folderToImplode, port) {
    const unusedStyles = await mainGetUnusedCss(folderToImplode);
}
