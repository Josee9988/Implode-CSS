let fs;_1bf‍.x([["getUnusedCss",()=>getUnusedCss],["default",()=>_1bf‍.o]]);_1bf‍.w("fs",[["default",["fs"],function(v){fs=v}]]);/**
 ...
 */
       function getUnusedCss(cssFiles, idsFoundHTML, classFoundHTML) {
    let idsInCss = [];
    let classInCss = [];
    const unusedArray = [];
    let classNotUsed = [];
    ...
    for (let i = 0; i < idsInCss.length; i++) {
        if (idsFoundHTML.indexOf(idsInCss[i].css[i].substr(1)) === -1) { // if it hasn't been used
            idsNotUsed = idsNotUsed.concat({
                css: idsInCss[i].css,
                path: idsInCss[i].path,
